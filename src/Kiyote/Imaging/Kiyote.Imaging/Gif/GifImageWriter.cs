...
public sealed class GifImageWriter : IImageWriter {

	private const int BlackAndWhiteMinCodeSize = 2;
	private const int GrayscaleMinCodeSize = 8;
	...
	private static void ThrowIfPixelTypeNotSupported<T>() {
	...
	private static void DoWriteImage<T>(
		...
		int colorTableSizeExponent = typeof( T ) == typeof( bool ) ? 0 : 7;
		int minCodeSize = typeof( T ) == typeof( bool ) ? BlackAndWhiteMinCodeSize : GrayscaleMinCodeSize;
	...
	private static void WritePalette<T>(
		Stream output
	) {
		if( typeof( T ) == typeof( bool ) ) {
	...
	private static void WriteImageData<T>(
		Stream output,
		IBuffer<T> pixels,
		int width,
		int height,
